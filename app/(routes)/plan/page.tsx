...
export default function PlanPage() {
  ...
  const lastUpdated = plan.lastUpdated ?? plan.weekOf;
    ...
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-stone/60">Last updated {new Date(lastUpdated).toLocaleString()}</p>
        </div>
        {offline && <Chip label="Offline" tone="warning" />}
      </header>
      ...
      <Card className="space-y-3">
        ...
        <p className="text-xs text-stone/60">
          Next session rationale: {plan.nextSessionRationale ?? 'Coach will refine the next session based on your feedback.'}
        </p>
